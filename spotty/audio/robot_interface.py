...
from spotty.audio import system_prompt_assistant, system_prompt_robin
...
class ChatClient:
    ...
    def text_to_speech(self, text: str) -> None:
        ...
        try:
            ...
            response = self.client.audio.speech.create(model="tts-1", voice="shimmer", input=text)
...
class WakeWordDetector:
    ...
    def _initialize_porcupine(self):
        ...
        try:
            ...
            self.recorder = PvRecorder(device_index=self.config.device_index, frame_length=self.porcupine.frame_length)
        except Exception as e:
...
class VoiceInterface:
    ...
    def _initialize_components(self) -> None:
        ...
        self.chat_client = ChatClient(
            system_prompt=self.config.system_prompt, max_context_length=self.config.max_context_length
        )
        self.audio_manager = AudioManager(AudioConfig())

        wake_word_config = WakeWordConfig(
            access_key=os.getenv("PICOVOICE_ACCESS_KEY"), keyword_path=self.config.keyword_path
        )
    ...
    def _process_voice_input(self) -> Optional[str]:
        ...
        """
        audio_file = self.audio_manager.record_audio(max_recording_time=self.config.max_recording_time)
